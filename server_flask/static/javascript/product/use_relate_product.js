...
function use_relate() {
    const product_id = document.getElementById('product_id_relate').value;
    const source_ids =  Array.from(
        document.querySelectorAll('#relate [name="source_id"]')
    ).map(el => el.value);
    const relate_quantity =  Array.from(
        document.querySelectorAll('#relate [name="relate_quantity"]')
    ).map(el => el.value);
    const product_ids = source_ids.map(function() {
        return product_id;
        });
    //  нам нужен id новго товара и старого де взять и функция закрузки етих товаров
    // потом взять все исходники и подготовить 
