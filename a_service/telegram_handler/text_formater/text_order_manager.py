...
class TextOrderManager:
    ...
    def builder(self):
        self.text_blocks.product_block_header = self.product_block_header(self.order)
        self.text_blocks.client_comment = self.client_comment(self.order)
        self.text_blocks.address_line = self.address_line(self.order)
        self.text_blocks.order_number_description = self.order_number_description(self.order)
        self.text_blocks.contact_number_phone_number = self.contact_number_phone_number(self.order)
        self.text_blocks.contact_name = self.contact_name(self.order)
        self.text_blocks.payment_method_sum = self.payment_method_sum(self.order)
